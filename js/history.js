  ...
  function render(){
    ...
    list.innerHTML=records.map(record=>{
      ...
      return `<article class="history-item ${record.favorite?'favorite':''}" data-record-id="${safe(record.id)}" data-reading-type="${safe(record.readingType||'reflection')}">
        <span class="tiny-label">${safe(record.date)} · ${safe(record.topic)} · ${record.drawMode==='fate'?'交给命运':'自选数字'}${record.readingType==='daily'?'<span class="daily-record-badge">本日运势</span>':record.readingType==='weekly'?'<span class="weekly-record-badge">本周运势</span>':''}</span><h3>${safe(record.question)}</h3><div class="history-card-row">${cards}</div>

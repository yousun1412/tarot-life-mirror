(() => {
  ...
  const cardData=(raw,recordDeckId)=>window.LifeMirrorDecks?.resolveRawCard(raw,recordDeckId)||window.LIFE_MIRROR_DATA?.cards?.find(card=>card.id===raw.id)||window.LIFE_MIRROR_DATA?.cards?.find(card=>card.name===raw.name)||raw;
  function snapshot(record){return {...record,cards:record.cards.map(raw=>{const card=cardData(raw,record.deckId);return {...raw,id:card.id,name:card.name,image:card.image,deckId:raw.deckId||record.deckId||'classic-rws',resolvedDeckId:raw.resolvedDeckId||card.resolvedDeckId||'classic-rws'};})};}
  function render(){
    ...
    list.innerHTML=records.map(record=>{
      const cards=record.cards.map(raw=>{const card=cardData(raw,record.deckId);return `<button class="history-card-mini ${raw.orientation==='reversed'?'reversed':''}" data-view-card="${card.id}" data-orientation="${raw.orientation}" data-deck-id="${safe(raw.deckId||record.deckId||'classic-rws')}" data-resolved-deck-id="${safe(raw.resolvedDeckId||card.resolvedDeckId||'')}" title="放大${safe(card.name)}"><img src="${safe(card.image)}" alt="${safe(card.name)}"><span>${safe(card.name)}${raw.orientation==='reversed'?'（逆）':''}${raw.deckNumber?` · #${raw.deckNumber}`:''}</span></button>`;}).join('');
      return `<article class="history-item ${record.favorite?'favorite':''}" data-record-id="${safe(record.id)}" data-reading-type="${safe(record.readingType||'reflection')}">
        <span class="tiny-label">${safe(record.date)} · ${safe(record.topic)} · ${safe(window.LifeMirrorDecks?.registry?.[record.deckId||'classic-rws']?.name||'经典韦特')} · ${record.drawMode==='fate'?'交给命运':'自选数字'}${record.readingType==='daily'?'<span class="daily-record-badge">本日运势</span>':record.readingType==='weekly'?'<span class="weekly-record-badge">本周运势</span>':record.readingType==='monthly'?'<span class="monthly-record-badge">本月运势</span>':record.spread==='choice-six'?'<span class="question-spread-badge">选择牌阵</span>':record.spread==='relationship-five'?'<span class="question-spread-badge">关系牌阵</span>':record.spread==='obstacle-four'?'<span class="question-spread-badge">阻力资源</span>':record.spread==='celtic-ten'?'<span class="question-spread-badge">凯尔特十字</span>':''}</span><h3>${safe(record.question)}</h3><div class="history-card-row">${cards}</div>
  ...
  function init(){
    ...
    list.onclick=e=>{
      const cardBtn=e.target.closest('[data-view-card]');if(cardBtn){window.LifeMirrorViewer?.open(Number(cardBtn.dataset.viewCard),cardBtn.dataset.orientation,cardBtn.dataset.deckId,cardBtn.dataset.resolvedDeckId);return;}

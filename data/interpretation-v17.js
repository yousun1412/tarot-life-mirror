...
(() => {
  ...
  const buildMultiSummary = ({ selected, positions, topic, question, topicNote }) => {
    const intro = `关于本周，${selected.length}张牌共同形成一张“${positions.map(position => position.label).join('—')}”的观察地图。`;
    const lines = selected.map((selection, index) => {
      const card = selection.card;
      const mark = selection.orientation === 'reversed' ? '逆位' : '正位';
      const core = selection.orientation === 'reversed' ? card.reversedCore : card.uprightCore;
    ...
    const action = selected[actionIndex >= 0 ? actionIndex : selected.length - 1];
    return [
      intro,
      ...lines,
      challenge ? `本周优先管理：${challenge.orientation === 'reversed' ? challenge.card.reversedAdvice : challenge.card.shadow}` : '',
      action ? `本周最先落实：${action.orientation === 'reversed' ? action.card.reversedAdvice : action.card.uprightAdvice}` : '',
      `综合方向：${topicNote}`
    ].filter(Boolean).join('\n\n');
  };

...
(() => {
  ...
  const buildMultiSummary = ({ selected, positions, topic, question, topicNote }) => {
    ...
    return [
      intro,
      ...lines,
      challenge ? `${topic === 'monthly' ? '本月' : '本周'}优先管理：${challenge.orientation === 'reversed' ? challenge.card.reversedAdvice : challenge.card.shadow}` : '',
      action ? `${topic === 'monthly' ? '本月' : '本周'}最先落实：${action.orientation === 'reversed' ? action.card.reversedAdvice : action.card.uprightAdvice}` : '',
      `综合方向：${topicNote}`
    ].filter(Boolean).join('\n\n');
  };
